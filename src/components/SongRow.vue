...
const props = defineProps(['track','artist','index'])
  ...
  >
    <div class="flex items-center w-full py-1.5">
        ...
        <div>
            ...
            <div class="text-sm font-semibold text-gray-400" :class="hoverStyle">
                {{ artist.name }}
            </div>
        </div>
    </div>
    <div class="flex items-cente">
        <button type="button" v-if="isHover">
            <Heart class="text-green-500" />
        </button>
    </div>
    <div v-if="isTrackTime" class="text-xs mx-5 text-gray-400" :class="hoverStyle">
        {{ isTrackTime }}
    </div>
  </li>
</template>
